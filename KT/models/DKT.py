...
class DKT_AUG(nn.Module):
    def __init__(self, feature_dim, question_num, skill_num,qs_matrix,embed_dim, hidden_dim, output_dim,augment_flag =False,dropout=0.2, bias=True):
        ...
        if augment_flag:
            self.rnn = nn.LSTM(self.embed_dim+self.skill_num+1, hidden_dim, bias=bias, dropout=dropout, batch_first=True)
        else:
